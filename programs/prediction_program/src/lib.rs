...
require!(market.status == MarketStatus::Open as u8, PredictionError::InvalidMarketStatus);
...
match outcome_index {
0 => require!(position.yes_shares >= shares_in, PredictionError::InsufficientShares),
1 => require!(position.no_shares >= shares_in, PredictionError::InsufficientShares),
_ => return err!(PredictionError::InvalidOutcome),
}
...
// Update reserves using net_out (what actually leaves the vault).
// The fee stays in the vault, so reserves should reflect that extra collateral retained.
// We already computed new_yes/new_no based on gross_out, so we reconcile by "keeping fee in the pool":
// - For YES sell, gross_out came from the opposite reserve (NO side). Keeping fee means NO decreases by net_out not gross_out.
// We'll do a small adjustment after the swap.
market.yes_pool = new_yes;
market.no_pool = new_no;

// Adjust reserve to account for fee retention
let fee_kept = gross_out.checked_sub(net_out).ok_or(PredictionError::MathOverflow)?;
if fee_kept > 0 {
match outcome_index {
0 => {
// YES sell pays out from NO reserve; keeping fee increases NO reserve
market.no_pool = market.no_pool.checked_add(fee_kept).ok_or(PredictionError::MathOverflow)?;
}
1 => {
// NO sell pays out from YES reserve; keeping fee increases YES reserve
market.yes_pool = market.yes_pool.checked_add(fee_kept).ok_or(PredictionError::MathOverflow)?;
}
_ => {}
}
}
...
match outcome_index {
0 => {
position.yes_shares = position.yes_shares.checked_sub(shares_in).ok_or(PredictionError::MathOverflow)?;
market.total_yes_shares = market.total_yes_shares.checked_sub(shares_in).ok_or(PredictionError::MathOverflow)?;
}
1 => {
position.no_shares = position.no_shares.checked_sub(shares_in).ok_or(PredictionError::MathOverflow)?;
market.total_no_shares = market.total_no_shares.checked_sub(shares_in).ok_or(PredictionError::MathOverflow)?;
}
_ => unreachable!(),
}
...
/// Claim winnings after resolution using pro-rata payout from vault.
///
/// payout = vault_balance * user_winning_shares / total_winning_shares
pub fn claim_winnings_v2(ctx: Context<ClaimWinningsV2>) -> Result<()> {
let market = &ctx.accounts.market;
let position = &mut ctx.accounts.position;

require!(market.status == MarketStatus::Resolved as u8, PredictionError::MarketNotResolved);
...
let total_winning_shares = match winning as u8 {
